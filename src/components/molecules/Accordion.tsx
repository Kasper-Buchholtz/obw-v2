...
const AccordionIcon: React.FC<{ isPanelOpen: boolean }> = ({ isPanelOpen }) => {
  return (
    <span
      className={`${isPanelOpen ? 'rotate-180' : 'rotate-0'} fill-superego-dark right-4 absolute w-auto fill-blues-dark-100 *:size-4 [&_svg_path]:!fill-superego-dark transition-all`}
    >
...
const AccordionPanel: React.FC<{ children: React.ReactNode }> = ({
  ...
  return (
    <div className="px-4 pb-6 text-superego-dark accordion__content has-text-wrap [&:not(:first-child)]:mr-4">
      {children}
    </div>
  )
}
