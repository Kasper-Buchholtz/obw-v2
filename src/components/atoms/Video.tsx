 ...
 * @example: <Video video={video} thumbnail={thumbnail} popup={true} />
 * @alias: Video
...
const Video = ({ video, thumbnail, popup }: VideoProps) => {
    return (
        <div className="relative h-full">
            <Photo image={thumbnail} />
            {popup ? <Popup video={video} /> : <VideoPreview video={video} />}
        </div>
    )
};
    ...
    <Dialog.Root>
        <Dialog.Trigger asChild>
            <button className="absolute translate-x-1/2 -translate-y-1/2 right-1/2 top-1/2 ">
                <svg className="m-auto transition-all bg-transparent rounded-full text-superego-green group-hover:bg-superego-green group-active:bg-transparent " width="100" height="100" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Play button"><rect className="text-current" x="2" y="2" width="96" height="96" rx="48" stroke="currentColor" strokeWidth="4" /><path className="text-current transition-all group-hover:text-superego-light-light group-active:text-superego-green" d="M69.245 48.2984C70.5104 49.0799 70.5104 50.9201 69.245 51.7016L41.051 69.1156C39.7185 69.9385 38 68.9801 38 67.414L38 32.586C38 31.0199 39.7185 30.0615 41.051 30.8844L69.245 48.2984Z" fill="currentColor" /></svg>
            </button>
        </Dialog.Trigger>
        <Dialog.Portal>
